...
// Registra apenas método e rota. Não registra token, senha nem conteúdo sensível.
...
async function requireAdmin(req, res, next) {
  try {
    // Não confia somente na role gravada no JWT.
    // Isso evita que um token antigo continue com role desatualizada.
    await connectDatabase();

    const databaseUser = await User.findById(req.user?.sub)
      .select("login role")
      .lean();

    if (!databaseUser) {
      return res.status(401).json({
        message: "Usuário da sessão não existe mais."
      });
    }
    ...
    // Mantém os dados atuais do banco disponíveis nas rotas seguintes.
    req.user.login = databaseUser.login;
    req.user.role = databaseUser.role;

    next();
  } catch (error) {
    console.error("ERRO AO VALIDAR ADMIN:", error);
    return res.status(500).json({
      message: "Não foi possível validar a permissão do administrador.",
      error: error.message
    });
  }
}
...
// Pode ser removido depois que o diagnóstico do 403 estiver concluído.
// ======================================================
app.get("/api/debug-auth", authenticate, async (req, res) => {
  try {
    await connectDatabase();
    ...
    return res.json({
      ...
      database: databaseUser
        ? { login: databaseUser.login, role: databaseUser.role }
        : null,
  ...
  upload.single("arquivo")(req, res, error => {
    ...
    if (error.code === "LIMIT_FILE_SIZE") {
      return res.status(413).json({
        message: "Arquivo muito grande. Limite local: 2,5 MB. Para vídeos maiores, use uma URL externa."
      });
    }
...
app.post("/api/documentos", authenticate, requireAdmin, uploadSingle, async (req, res) => {
  console.log("POST /api/documentos chegou ao Express", {
    login: req.user?.login,
    role: req.user?.role,
    temArquivo: Boolean(req.file),
    tipoConteudo: req.body?.tipoConteudo || null
  });

  try {
    await connectDatabase();
    const payload = await buildDocumentPayload(req);
    payload.agente = req.user.login;
...
app.put("/api/documentos/:id", authenticate, requireAdmin, uploadSingle, async (req, res) => {
  console.log("PUT /api/documentos/:id chegou ao Express", {
    login: req.user?.login,
    role: req.user?.role,
    id: req.params.id,
    temArquivo: Boolean(req.file),
...
// ======================================================
// ERROS NÃO TRATADOS DA API
// Garante JSON quando o erro já chegou ao Express.
// ======================================================
